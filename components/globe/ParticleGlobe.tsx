...
function GlobeScene({ onClickCountry, activeCountryCode, particleData }: {
  ...
  useEffect(() => {
    ...
    function loop() {
      ...
      if (mainRef.current) mainRef.current.rotation.y += delta * 0.175;
      ...
      <group ref={mainRef}>
        ...
        <group ref={edgeGrp}>
          <points ref={edgeMesh}>
            ...
            <pointsMaterial color="#00e5d0" size={0.008} transparent opacity={0.5} depthWrite={false} blending={THREE.AdditiveBlending} />
          </points>
        </group>
        <group ref={glowGrp}>
          <points ref={glowMesh}>
